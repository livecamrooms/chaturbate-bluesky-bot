...
def make_hashtags(tags_list):
    """Convert room tags → clean #Hashtags (max 5)"""
    hashtags = []
...
def main():
    ...
    for room in filtered[:MAX_POSTS_PER_RUN]:
        try:
            ...
            # === NEW: Rich text with clickable "Watch FREE" link ===
            tb = client_utils.TextBuilder()
            tb.text(f"🔥 {niche} LIVE NOW ({room['num_users']} watching)\n\n")
            tb.text(f"{room['username']} • {room.get('age') or '?'} • {room.get('country') or '??'}\n")
            tb.text(f"{subject}\n\n")
            tb.link("👉 Watch FREE", room['chat_room_url_revshare'])   # <--- This makes it clickable!
            tb.text(f"\n\n#Chaturbate #{niche} #CamGirls #LiveCams #Adult {extra_tags}")

            client.send_image(
                text=tb,                                      # Pass TextBuilder instead of str
                image=img_bytes,
                image_alt=f"Live HD thumbnail of {niche} {room['username']} - {subject[:50]}"
            )

            print(f"Posted: {niche} - {room['username']} | Tags: {extra_tags}")
            time.sleep(15)

        except Exception as e:
            print(f"Error: {room['username']} - {e}")
